...
use crate::error::*;
use crate::heartbeat::{spawn_heartbeat_task, ActivityTracker, HeartbeatConfig, HeartbeatHandle};
                        ...
                        Some(Ok(Message::Command(cmd))) => {
                            ...
                            match cmd.name {
                                ZmqCommandName::PING => {
                                    ...
                                    if let Some(mut peer_send) =
                                        self.backend.peers.get_async(&peer_id).await
                                    {
                                        if let Err(e) =
                                            peer_send.send_queue.send(Message::Command(pong)).await
                                        {
                                            log::warn!(
