        ...
        loop {
            match self.fair_queue.next().await {
                ...
                Some((peer_id, Ok(Message::Command(cmd)))) =>
                {
                    ...
                    match cmd.name {
                        ZmqCommandName::PING => {
                            if let Some(mut subscriber) =
                                self.backend.subscribers.get_async(&peer_id).await
                            {
                                let pong = ZmqCommand::pong(cmd.context.clone());
                                if let Err(e) =
                                    subscriber.send_queue.send(Message::Command(pong)).await
                                {
